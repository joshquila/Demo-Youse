...
<?php endif; // is_taxonomy_hierarchical() ?>
		<tr class="form-field">
			<th scope="row" valign="top"><label for="description"><?php if($taxonomy=='product_cat'){echo 'Shirt Front View SVG';}else{_ex('Description', 'Taxonomy Description');} ?></label></th>
			<td><textarea name="description" id="description" rows="10" cols="50" class="large-text"><?php echo $tag->description; // textarea_escaped ?></textarea>
			<?php if($taxonomy!='product_cat'){?><br />
			<span class="description"><?php _e('The description is not prominent by default; however, some themes may show it.'); ?></span>
			<?php }?></td>
		</tr>
<?php if($taxonomy=='product_cat'){?>
<tr class="form-field">
			<th scope="row" valign="top"><label for="description">Shirt Back View SVG</label></th>
			<td><textarea name="description_back" id="description_back" rows="10" cols="50" class="large-text"><?php echo $tag->description_back; // textarea_escaped ?></textarea>
			</td>
		</tr>
<tr class="form-field">
			<th scope="row" valign="top"><label for="description">Note</label></th>
			<td><textarea name="cat_note" id="cat_note" rows="5" cols="50" class="large-text"><?php echo $tag->cat_note; // textarea_escaped ?></textarea>
			</td>
		</tr>
		<?php }?>
		<?php
