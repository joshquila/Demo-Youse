...
{
	$lines = file($url);
	$line_code_str = implode('',$lines);
	$line_code_arr = explode('=======================================================',$line_code_str);
	$guessurl = wp_guess_url();
	//echo $guessurl;
	$folder = explode('/',$guessurl);
	$folderpath = '';
	if(isset($folder[3]) && !empty($folder[3])){
		//echo count($folder);
		for($t=3;$t<count($folder);$t++){
				$folderpath .= $folder[$t].'/';
		}
	}
	//print_r($folderpath);
	//die();
	foreach($line_code_arr as $line_code_arrqu){
		$line_code_arrqur = str_replace('http://teecircle.com',$guessurl,$line_code_arrqu);
		mysql_query($line_code_arrqur);
		//echo $line_code_arrqur.'<br /><br />';
	}
	...
	$data = '#BEGIN create a mirror of wp-admin folder
RewriteEngine on
RewriteBase /
